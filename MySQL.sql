...
MODELAGEM LOGICA - PROGRAMAS COMO BR.MODELO
...
/*VERIFICANDO AS TABELAS  -  Mostra as tabelas existentes no banco -  Nao mostra as colunas, apenas o nome da tabela */

SHOW TABLES; #so no mysql

/* DESCREVENDO TABELAS - Mostra tabela com todos as colunas e dados inseridos */
...
#select é o comando mais importante do SQL

SELECT NOW();  #Retorna data e horario do computador

SELECT 'FELIPE MAFRA';

SELECT 'BANCO DE DADOS';

/* ALIAS DE COLUNAS */  # pesquisa por colunas

SELECT 'FELIPE MAFRA' AS PROFESSOR; # esse AS cria uma nova coluna

#observe que nao cadastramos esse nome no banco de dados - O select serve pr trazer qualquer coisa!
...
#Usar * dexa a aplicação mais lenta - leva mta coisa desencessaria
#O * ignora qualquer tipo de indexação
...
#filtro é feito por meio da clausula WHERE
...
SELECT NOME, SEXO FROM CLIENTE
WHERE ENDERECO = 'RJ'; #erro, pois nenhum edenereço é igual a RJ, quando tratamos de = entao sera feita a bsuca para encontrar exatamente igual aquilo

/* UTILIZANDO O LIKE */

#like = como
#vai buscar a palavra no meio de todo o campo

SELECT NOME, SEXO FROM CLIENTE
WHERE ENDERECO LIKE 'RJ'; # erro, pois falta o %

/* CARACTER CORINGA % */

# % = tudo antes ou tudo apos

SELECT NOME, SEXO FROM CLIENTE
WHERE ENDERECO LIKE '%RJ'; # comeca com qualquer coisa e termina com RJ

SELECT NOME, SEXO, ENDERECO FROM CLIENTE
WHERE ENDERECO LIKE '%RJ';

SELECT NOME, SEXO, ENDERECO FROM CLIENTE
WHERE ENDERECO LIKE '%CENTRO%'; #começa com qualquer coisa - Centro - termina com qualquer coisa

# O Like é inimigo da performance, deixa a performance baixa
#like deve ser usado em casos extremos
...
#Complementa a clausula Where - na maioria das vezes queremos filtrar por mais de 1 condição

#Banco de dados é teoria de conjunto
...
SELECT COUNT(*) FROM CLIENTE;  #o nome da coluna aparece como count


SELECT COUNT(*) AS "QUANTIDADE DE REG TAB CLIENTE"
FROM CLIENTE;  #Para nao ficar feio, entao usar AS para mudar o nome da coluna



/* OPERADOR GROUP BY */

#Group By serve para agrupar por determinada coluna da tabela
#Sempre usado em conjunto com outra função
...
OR CIDADE = 'RIO DE JANEIRO';
 ...
 -- Coloca o Sexo na frente, pois como é OU e basta 1 V para o resultado ser vdd
 entao ao colocar o que tem mais - 70% - voce faz com que so seja verificada apenas a primeira condição
 e apenas se for Masculino e entao checar a segunda condição
...
-- Ja no caso do AND, como as duas precisam ser V, entao voce coloca a quee tem menos chance de ser verdadeira
na frente, pq se der F entao nem olha a segunda condição

/* A13 - FILTRANDO VALORES NULOS */

SELECT NOME, SEXO, ENDERECO
FROM CLIENTE
WHERE EMAIL = NULL; # da erro

# ao filtrar valor nulo nao tem como igualar

SELECT NOME, SEXO, ENDERECO
FROM CLIENTE
WHERE EMAIL IS NULL;

#Entao se troca o = por IS - Onde o email é nulo

SELECT NOME, SEXO, ENDERECO
FROM CLIENTE
WHERE EMAIL IS NOT NULL; #Buscando quais nao sao nulos
...
DELETE FROM CLIENTE; -- NUNCA FAÇAM DELETE SEM WHERE - Pois ele vai deleltar tudo
...
WHERE NOME = 'JOAO'; #Esse caso vai mudar o telefone q esta no banco

UPDATE CLIENTE 
SET TELEFONE = '22923110 - 97865434'
WHERE NOME = 'JOAO'; #Os dois serao armazenados no banco, mas ao realzar o Count, os dois serao considerados como um só - Em um campo tera 2 telefones

SELECT ENDERECO, COUNT(*) AS "QUANTIDADE"
FROM CLIENTE
GROUP BY ENDERECO;

/*	Modelagem é umportante para garantir que se consiguira retirar informações necessarios -
ex: Modelamos o campo endereço de forma em que a cidade, bairro, estado... estao no mesmo campo(endereço), dessa forma ao tentar extrair
determinadas informações nao conseguiremos, por exemplo: Agrupar endereços: nao serao agrupados pois, por mais que varios sejam do RJ, 
o restante do endereço é diferenca - assim o group by vai reconhecer todos como diferente - Centro - Rj é diferente de Barra -Rj logo nao agrupara
*/

#Modelagem errada gera informações erradas
...
ESCREVEU, TAO POUCOS ERAM OS PROGRAMADORES E tAO ARCAICO ERAM
...
POR EXEMPLO, ARMAZENAMOS NO BANCO A VENDA DE JANEIRO, FEVEREIRO E MARCO(Dados)
O TOTAL DAS VENDAS E O IMPOSTO PAGO AO GOVERNO(INFORMACOES). 
...
VETOR = ELEMENTOS DE UM MESMO TIPO 

[AZUL,AMARELO,VERDE,LARANJA] - Vetor de cores
[KA,FIT,FIESTA,CIVIC,UNO] - VETOR DE CARROS

Entao teremos uma tabela de telefone

2 - TODO CAMPO MULTIVALORADO SE TORNARÁ
OUTRA TABELA. UM CAMPO DIVISIVEL.

Elementos nao sao de mesma natureza
	ex: Endereço - temos rua, bairro, cidade e estado - temos 4 
	valores em um campo - ou seja campo multivalorado
...
Todo registro(linha) precisa ter pelo menos um campo(Coluna) que o defina como unico
	No caso, o nosso seria CPF - mas isso nao é uma boa ideia - pois nao devemos
	nunca modelar baseado em procedimentos, principalmente procedimentos externos - pois 
	procedimentos sofrem alterações, por exemplo o cpf pode acaber um dia, coo ja foi proposto um dia

Chave Primaria
	Chave Natural - provem de algo
		ex: CPF - provem de uma pessoa
		Nao é muito usado, pois se o negocio mudar entao vc precisa mudar todo o banco
	Chave Artificial - algo criado
		Ex: cliente_id
		Hj em dia se usa mais a artificial
...
ex: Telefone(0,n) - Possui - (1,1)Cliente
	Telefone:
		0 - nao é obrigaorio
		n - pode ter mais de 1 por cliente
	Cliente
		1 - é obrigatorio
		1 - Um cliente so pode ser ele mesmo - max de 1 cliente por cadastro - nao da pra cadastrar 2 pessoas de uma vez so
Ex: ex: Endereco(1,1) - Possui - (1,1)Cliente
	Enereço:
		1 - Obrigatorio
		1 - So pode ter 1 endereço por cliente
	Cliente
		1 - é obrigatorio
		1 - Um cliente so pode ser ele mesmo - max de 1 cliente por cadastro - nao da pra cadastrar 2 pessoas de uma vez so
...
CREATE TABLE CLIENTE(
	...
	NOME VARCHAR(30) NOT NULL,  #Obriga a ter o nome no insert, caso campo esteja vazio entao sistema tera erro
	SEXO ENUM('M','F') NOT NULL,
	EMAIL VARCHAR(50) UNIQUE,  #nao existe 2 emails iguais, cada email é unico
	CPF VARCHAR(15) UNIQUE #nao existem 2 pessoas com o mesmo cpf
);

#Unique - nao permite que a coluna tenha valores repetidos
...
#Tabela mais fraca é aquela que nao existe sem outra
#ex: Endereco nao existe sme cliente, entao tbela enereco é a mais fraca
...
#Isso facilita a busca do banco, ao dizer q a coluna esta na tabela cliente(cliente.nome), o banco ja vai direto em cliente

#Apelido para tabela - C apelido para Cliente
SELECT C.NOME, C.SEXO, 
E.BAIRRO, E.CIDADE, 
T.TIPO, T.NUMERO
FROM CLIENTE C #defininfo o apelido
INNER JOIN ENDERECO E #defininfo o apelido
ON C.IDCLIENTE = E.ID_CLIENTE
INNER JOIN TELEFONE T #defininfo o apelido
...
FROM CLIENTE C
INNER JOIN TELEFONE T
ON C.IDCLIENTE = T.ID_CLIENTE  #quando chave estrangeira for igual a chave primaria
INNER JOIN ENDERECO E
ON C.IDCLIENTE = E.ID_CLIENTE;
...
#Quando for usar uma query varias vezes
#Tem menos perfomance
...
SELECT NOME, SEXO, CIDADE  #Projeção
FROM V_RELATORIO;
...
Can not delete from join view 'comercio.v_relatorio' #Pois se trata de um relatorio e dessa forma as fk dependem de outras info
...
INNER JOIN ENDERECO
ON IDCLIENTE = ID_CLIENTE
ORDER BY NOME, CPF ASC; #Primeiro ordena por nome e em caso de nome igual entao ordena pelo cpf,  no caso de forma ascendente

SELECT NOME, SEXO, CPF, CIDADE
FROM CLIENTE
INNER JOIN ENDERECO
ON IDCLIENTE = ID_CLIENTE
ORDER BY 4 #ordena pela coluna, no caso seria a coluna 4 - Cidade
...
/*TRUNCATE - Truca os valores - pede dois parametros - 1- Um numero 2- Numero de casas decimais*/
...
WHERE MARCO = (SELECT MIN(MARCO) FROM VENDEDORES); -- O que esta entre () é a inner query

/*
A inner query é resolvida antes - dessa forma primeiro ele vai pegar 
o menor valor de março e depois faz o resto normal - ou seja - seleciona
nome e marco de vendedores em que marco é igual a 4496
Dessa forma fica:
SELECT NOME, MARCO FROM VENDEDORES
WHERE MARCO = 4496;
*/
...
/*
O resultado na inner query tem q ser igual ao numero de condição da alter query
ex: where marco = inner quer(1 resultado)
ex: where marco and feve = inner query(2 resultados )
*/
